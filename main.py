...
class EMASmoother:
    def __init__(self, rate=0.5, dimension=45):
        self.rate = rate
        self.dimension = dimension
        self.value = np.zeros(dimension)

    def forward(self, value):
        self.value = self.value * (1 - self.rate) + np.asarray(value) * self.rate
...
def main():
    ...
    eyebrow_smoother = EMASmoother(0.5, 12)
    eye_mouth_smoother = EMASmoother(0.75, 27)
    pose_smoother = EMASmoother(0.1, 6)
    position_smoother = EMASmoother(0.2, 4)
    ...
    while True:
        ...
        position_vector = position_smoother.forward(position_vector)
